...
**NOTE: Based on feedback received we investigated an issue with excessive load on Gateways performing load monitoring using these include files. As a result of these investigations we would advise avoiding the use of these files until the release of Geneos 6.4.0 which makes two important changes to the way load monitoring is handled. In testing these changes improved performance markedly when monitoring large, real-world Gateways.**
